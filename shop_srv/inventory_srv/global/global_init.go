...
func InitConsul() (*api.Client, string) {
	data, err := yaml.Marshal(Nacos["inventoryServer"].(map[string]interface{}))
	if err != nil {
		zap.S().Panic(err)
	}
...
func InitRPCServer(g *grpc.Server) {
	port := strconv.Itoa(Nacos["inventoryServer"].(map[string]interface{})["port"].(int))
...
func InitViper() {
	// 初始化Viper
	viper.SetConfigName("inventory_srv") // 配置文件名（不带扩展名）
