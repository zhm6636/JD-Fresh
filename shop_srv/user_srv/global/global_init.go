...
func InitConsul() (*api.Client, string) {
	data, err := yaml.Marshal(Nacos["userServer"].(map[string]interface{}))
	if err != nil {
		zap.S().Panic(err)
	}
...
func InitRPCServer(g *grpc.Server) {
	port := strconv.Itoa(Nacos["userServer"].(map[string]interface{})["port"].(int))
	...
	zap.S().Infof("user_srv start success listen on " + port)
...
func InitViper() {
	// 初始化Viper
	viper.SetConfigName("user_srv") // 配置文件名（不带扩展名）
